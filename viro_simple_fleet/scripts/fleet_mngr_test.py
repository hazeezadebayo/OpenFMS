...
class TestFleetManager:
    ...
    def set_active_agv(self, robot_id, fleet_id):
        """ timer callback function that updates db and nav variables. """
        self.fm_main.fm_start_fleet(fleet_id)
        self.fm_main.fm_start_robot(robot_id)
        #time.sleep(1.5)

    def shutdown_trigger(self, val:bool):
        """ fm_shutdown_trigger """
        # answer = input('Alert! check yes or no for on or off? [yes, no]: ')
        self.fm_main.fm_shutdown_trigger(val)
        #time.sleep(1.5)

    def robot_status_trigger(self, val:bool):
        """ fm_robot_status_trigger """
        # answer = input('Alert! check yes or no for active or inactive? [yes, no]: ')
        self.fm_main.fm_robot_status_trigger(val)
        #time.sleep(1.5)

    def manual_control_trigger(self, val:bool):
        """ fm_manual_control_trigger """
        # answer = input('Alert! check yes or no to nav manually or not? [yes, no]: ')
        self.fm_main.fm_manual_control_trigger(val)
        #time.sleep(1.5)

    def manual_control_drive(self, choice:int):
        ...
        if not 1 <= int(choice) <= len(options):
            ...
            self.fm_main.fm_manual_control_drive(chosen_option)

    def send_task_request(self, from_loc_id, to_loc_id, task_name, task_priority):
        ...
        # ---
        self.fm_main.fm_send_task_request(from_loc_id, to_loc_id, task_name, task_priority)
        time.sleep(2.0)

    def clear_task_trigger(self, val:bool):
        """ fm_clear_task_trigger """
        # answer = input('WARNING! Are you sure you want to clear task and make robot idle? [yes, no]: ')
        self.fm_main.fm_clear_task_trigger(val) # True
        time.sleep(2.0)

    def add_landmark_request(self, station_type:str, input_loc_id:str, neighbor_loc_ids:str):
        ...
        # ----
        self.fm_main.fm_add_landmark_request(station_type, input_loc_id, neighbor_loc_ids)
        #time.sleep(1.5)
    ...
    def setup_robot(self, robot_id, fleet_id, current_pose, drop_table=False):
        """ test create robot """
...
# ---------------------------------------------- #
    ...
    r1_db_mngr = fm.setup_robot(ROBOT1_ID, FLEET1_ID, R1_CURRENT_POSE, True)
...
# ---------------------------------------------- #
    ...
    r2_db_mngr =fm.setup_robot(ROBOT2_ID, FLEET2_ID, R2_CURRENT_POSE)
...
# ---------------------------------------------- #
    ...
    fm.set_active_agv(ROBOT1_ID, FLEET1_ID)
    print("active robot set to robot 1.")
    print(" --        --        --        --")
    # clear robot task
    # ------------
    fm.clear_task_trigger(val=True)
    print("robot 1 task cleared.")
    print(" --        --        --        --")
    fm.send_task_request(from_loc_id="A5",
                         to_loc_id="A3",
                         task_name="transport",
                         task_priority="high")
...
# ---------------------------------------------- #
    ...
    ###########################################
    # time.sleep(5.0)
    # fm.set_active_agv(ROBOT2_ID, FLEET2_ID)
    ...
    # print(" --        --        --        --")
    # fm.send_task_request(from_loc_id="A3",
    #                      to_loc_id="A5",
    #                      task_name="transport",
    #                      task_priority="low")
...
# ---------------------------------------------- #
    ...
    # fm.robot_status_trigger(val=True)
