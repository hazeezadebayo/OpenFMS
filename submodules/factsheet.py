...
class FactsheetSubscriber:
    ...
    def __init__(self, fleetname, versions, dbconn, mqttclient=None,
        ...
        # In-memory cache — keyed by serialNumber, holds the raw MQTT factsheet payload.
        # Factsheets are published once at robot startup and rarely change.
        # Overwritten on each new factsheet message from the same robot.
        self.cache: dict = {}  # { r_id: raw MQTT factsheet payload }
    ...
    def process_message(self, message):
        ...
        try:
            ...
            # Update in-memory cache before DB write.
            r_id = message.get("serialNumber")
            if r_id:
                self.cache[r_id] = message
