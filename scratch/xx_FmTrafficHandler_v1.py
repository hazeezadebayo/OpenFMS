...
# ----------------------------------------------------------------------------
    ...
    def _handle_robot_traffic_status(self, f_id, _r_id, m_id, v_id, traffic_control: list[str], mex_record, ctx):
        ...
        has_order_minute_passed = self.check_minute_passed(ctx.order_timestamp, 0.15) # 0.25

        # Decide label based on whether reserved_checkpoint is in landmarks
        checkpoint_type = "Station" if reserved_checkpoint in ctx.landmarks else "Checkpoint"
        
        # Log current status (critical information)
        log_msg = (
            f"\nR_id: {_r_id}. \n"
            f"{checkpoint_type}: "   # <-- only one of them
            f"Reserved_node: {reserved_checkpoint}, "
        ...
        # Only append isDocked if reserved_checkpoint is in landmarks
        if reserved_checkpoint in ctx.landmarks:
            log_msg += f"isDocked: {ctx.dock_action_done}."

        # log viz:
        if _r_id == "R02": self.task_handler.visualization_handler.terminal_log_visualization(
            log_msg,
            "FmTrafficHandler",
        ...
        try:
            ...
            # agv_status=='red' means the robot has physically arrived at its base node
            # (distance check is now encoded in agv_status inside fetch_mex_data).
            ...
            if conditions_met:
                ...
                if ctx.merged_nodes[0].startswith('W'):
                    ...
                    print(f"------------big {_r_id} smoke-------------")
                    return

                # Check if next stop is occupied in the expanded_traffic_control
                elif next_stop_id in expanded_traffic_control:
                    # log viz:
                    self.task_handler.visualization_handler.terminal_log_visualization(
    ...
    def _handle_waitpoint_case(self, f_id, _r_id, m_id=None, v_id=None, ctx=None):
        ...
        print(f"\033[93m 🥈 checkpoints: {ctx.checkpoints} 🛸 \033[0m")
        print(f"\033[93m 🥈 release_node: {ctx.horizon} 🛸 \033[0m")
        ...
        try:
            ...
            b_node, h_nodes, h_edges = self.task_handler.order_handler.create_order(
                                                            ctx.checkpoints, # [1:], # = [ctx.base]+ctx.horizon, # ctx.checkpoints, # checkpoints
                                                            ...
                                                            ctx.horizon, # [ctx.base]+ctx.horizon # release (next_stop_id)
                                                            temp_fb_node_eta) # time of arrival at the newly released node
            self.task_handler.order_handler.build_order_msg(
                                                f_id,
                                                _r_id,
                                                header_id,
                                                v_id,
    ...
    def fetch_mex_data(self, f_id: str, r_id: Optional[str] = None, m_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        for order_rec in order_recs:
            try:
                ...
                # set agv status based on physical proximity to the robot's self-reported
                # current node (state_base), not the master's order position.
                # 'red'   = robot has arrived at its base node -> ready for a new order.
                # 'green' = robot is still travelling.
                order_base = checkps[1]
                state_base = state_rec.get("base", '')
                agv_pos = state_rec.get("position", [0.0, 0.0, 0.0, 0.0])
                agv_sz  = state_rec.get("agv_size", [0.5, 0.8])  # [width, length]
                # Look up state_base coordinate from the current order's itinerary.
                # Mirrors _get_base_coordinate logic but uses local loop variables.
                if state_base.startswith('W') and state_base in waitps:
                    base_coord = w_pnts_pos[waitps.index(state_base)]
                elif state_base in checkps:
                    base_coord = c_pnts_pos[checkps.index(state_base)]
                else:
                    base_coord = agv_pos  # fallback: treat robot as at its reported position
                dist_to_base = self._calculate_distance(
                    float(agv_pos[0]), float(agv_pos[1]),
                    float(base_coord[0]), float(base_coord[1])
                )

                
                # when state and order are same, robot is either about to reach order node or at order node.
                state_rec["agv_status"] = "red" if ((dist_to_base <= (1.5 * float(agv_sz[1]))) \
                    or (state_base == order_base)) else "green"
                

                print(f"1. Robot : {serial_number} : -> state_base : {state_base} & order_base : {order_base}.")




                if state_base.split(',')[0] != '': 
                    state_rec["base"] = state_base.split(',')[0]
                    # is the first element of the node_ids approved for me or not. did i reserve it?
                    # print(f"2. Robot : {serial_number} : -> Base[0] : {state_base.split(',')[0]}.")
                    ...
                    if node_ids[0].startswith('C'):
                        if checkps_release[0]:
                            ...
                            # print(f"3. Horizon : {checkps[1:]} : -> H_release : {checkps_release[1:]}.")
                        # if it is not, its an horizon:
                            ...
                            # print(f"4. Horizon : {checkps} : -> H_release : {checkps_release}.")
                    else: # we have a waitpoint wx cx cy wy cz wz. e.g. w17, c17 and c18
                        if len(node_ids) >= 3:
                            state_rec["horizon"] = checkps[1:] # [2:] # [0] c17, [1] c18, then.. horizon[2:]
                            state_rec["horizon_release"] = checkps_release[1:]  # [2:]
                            if serial_number == "R02":print(f"5. Horizon : {checkps[1:]} : -> H_release : {checkps_release[1:]}.")
                        else:
                    ...
                    # print(f"6. Base {checkps[0]} : Horizon : {checkps} : -> H_release : {checkps_release}.")

                print(f"\033[93m 🐢 checkpoints: {node_ids} 🛸 \033[0m")
